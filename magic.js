...
class Env {
  ...
  async ["run"](iiIIilii = {
    ...
    try {
      ...
      console.log("MK集团本部TG频道:https://t.me/Bigyellowtl");
  ...
  async ["auth"]() {
    if (this.activityUrl) {
      ...
      try {
        let {
          data: l1II111
        } = await this.request("http://127.0.0.1:7705/auth", {
          "Cookie": "authority",
          "token": apiToken,
  ...
  async ["sign"](liiIill, lil1lii1 = {}) {
    ...
    try {
      let {
        data: ilIIIi
      } = await this.request(signMode.includes("server") ? "http://192.168.5.89:17840/sign" : apiSignUrl, IlI1Il11, lll11lii);
      return {
        "fn": ilIIIi.fn,
        "sign": ilIIIi.body
      };
